    ...
    <section class="content">
        <form action="<?php echo asset('admin/accountant/edit/'.$accountant->id); ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                <div class="col-md-6" id='time_deposit'>
                    {{-- ngay Mua --}}
                    <div class="form-group depositTime">
                        <label>Ngày Cọc</label>
                        @php
                        $buyTime = date_create($accountant->buyTime);
                        $buyTime = date_format($buyTime, 'd/m/Y');
                        @endphp
                        <input type="text" class="form-control depositTime" name="buyTime"
                            id="depositTime_{{ $accountant->id }}" value="{{$buyTime}}">
                    </div>
                    <script>
                        $('#depositTime_' + {{ $accountant->id }}).datepicker({
                            format: 'mm/dd/yyyy',
                        });
                    </script>
                </div>
