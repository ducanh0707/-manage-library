    ...
    <section class="content">
        <form action="<?php echo asset('admin/accountant/edit/'.$accountant->id); ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                <div class="col-md-6" id='time_service'>
                    
                    {{-- ngay ngay nop --}}
                    <div class="form-group">
                        <label>Ngày nộp</label>
                        @php
                        $startTime = date_create($accountant->startTime);
                        $startTime = date_format($startTime, 'd/m/Y');
                        @endphp
                        <input type="text" class="form-control endTime" name="startTime"
                        id="startTime_{{ $accountant->id }}"   value="{{ $startTime }}">
                    ...
                    <div class="form-group">
                        ...
                        @php
                        $endTime = date_create($accountant->endTime);
                        $endTime = date_format($endTime, 'd/m/Y');
                        @endphp
                        <input type="text" class="form-control endTime" name="endTime"
                            id="endTime_{{ $accountant->id }}" value="{{ $endTime }}">
                    </div>
                    <script>
                        $('#endTime_' + {{$accountant->id }}).datepicker({
                            format: 'mm/dd/yyyy',
                        });
                        $('#startTime_' + {{$accountant->id }}).datepicker({
                            format: 'mm/dd/yyyy',
                        });
