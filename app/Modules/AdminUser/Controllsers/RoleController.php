...
class RoleController extends Controller {
    public function post_role_new(Request $request){
        ...
        $user_type -> commission = $request -> commission;
        $user_type -> parent_id = $request -> parent_id;
        $user_type -> save();
        // them quyen
    ...
    public function get_role_del($id){
        ...
        M_user_type::where('parent_id',$id)->update(['parent_id'=>0]);
    ...
    // xoa quyen
     ...
     public function get_role_permission($user_type_id){
        ...
        $permission = M_permission::where('type','admin')->get();
        $agency_permission = M_permission::where('type','agency')->get();
        $title = 'Danh sách quyền của thể loại thành viên'; 
        return view('AdminUser::role_permission',['agency_permission'=>$agency_permission,'permission'=>$permission,'user_type_id'=>$user_type_id,'title'=>$title]);
    }
