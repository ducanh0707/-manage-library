...
class ThemeController extends Controller{
    ...
    public function post_theme_new(Request $request){
        $theme = new M_theme;
        $theme -> title = $request -> title;
        $theme -> folder = $request -> folder;
        $theme -> file_name = $request -> file_name;
        $theme -> type = 'theme';
        $theme -> save();
        return redirect('admin/theme/'.$theme->id.'/row/0')->with('alert','Bạn đã thêm thành công');
    }
    public function get_theme_del(Request $request,$theme_id){
        // check row 
        $theme_row = M_theme_row::where('theme_id',$theme_id)->first();
        if($theme_row){
            return redirect()->back()->with('alert','Bạn vui lòng xóa hàng trước khi xóa giao diện');
        }
        M_theme::where('id',$theme_id)->delete();

        return redirect('admin/theme/1/row/1')->with('alert','Bạn đã xóa thành công');
    }
    public function row(Request $request,$theme_id,$row_id){
        ...
        $row = M_theme_row::where('theme_id',$theme_id)->where('id',$row_id)->first();
        ...
        $cat_list = M_cat::get();
        $type_course = M_post_type::where('url','course')->first();
      ...
      return view('Theme::theme_row',['theme_id'=>$theme_id,'row_first_id'=>$row_first_id,'sidebar_first_id'=>$sidebar_first,'cat_list'=>$cat_list,'row'=>$row,'row_id'=>$row_id,'menu_list'=>$menu_list,'cat_list'=>$cat_list,'slide_list'=>$slide_list,'row_list'=>$row_list,'title'=>'Cấu hình cơ bản website','theme'=>$theme]);
    }
    ...
    public function post_theme_row_new(Request $request,$theme_id){
        $this -> validate($request, [ 'name' => 'required', ], [ 'name.required' => 'Bạn chưa nhập tên row', ]);
         ...
         $theme_row -> theme_id = $request -> theme_id;
     ...
     public function post_theme_row_order(Request $request,$theme_id){
        ...
        foreach($order as $order_ex){
           ...
           foreach($order_id as $order_in){
              $row = M_theme_row::where('theme_id',$theme_id)->where('id',$order_in[1])->update(['orderby'=>$i]);
              $i++;
           }
        }
     }
